...
def format_dashboard(data: dict) -> str:
    ...
    text = (
        ...
        f"<code>  Cash Balance : Ksh {net_balance:,.2f}</code>\n"
        f"<code>  Portfolio    : Ksh {data.get('total_worth', 0.0):,.2f}</code>\n"
        f"<code>  Net Worth    : Ksh {(net_balance + data.get('total_worth', 0.0)):,.2f}</code>\n"
